...
public class HotItemApp {
    public static void main(String[] args) throws Exception {
        ...
        //读取文本数据创建流，转化为JavaBean，同时提取数据中的时间戳生成watermark
        ...
        //开窗，滑动窗口，一个小时窗口大小，五分钟的滑动步长
        WindowedStream<UserBehavior, Long, TimeWindow> windowedStream = keyedStream.timeWindow(Time.hours(1), Time.minutes(5));

        //聚合计算，计算窗口内部每个商品的点击次数（滚动聚合要求保证来一条计算一条，窗口函数保证可以拿到窗口时间）
        SingleOutputStreamOperator<ItemCount> itemCountWithEndDS = windowedStream.aggregate(new ItemCountAggFunc(), new ItemCountWindowFunc());

        //按照窗口时间分组
        KeyedStream<ItemCount, Long> windowEndKeyedStream = itemCountWithEndDS.keyBy(ItemCount::getWindowEnd);

        //使用ProcessFunction实现收集每个窗口中的数据并作排序输出(状态变成 -》 ListState 定时器)
        SingleOutputStreamOperator<String> result = windowEndKeyedStream.process(new ItemKeyedProcessFunc());
